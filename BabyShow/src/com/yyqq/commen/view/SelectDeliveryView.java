 ...
 * 玩具世界支付页 - 送货时间选择
 * */
public class SelectDeliveryView extends View {
	...
	// 监听器
	private OnSelectTimeViewItemSelectListener listener;

	public SelectDeliveryView(Context context) {
		super(context);
	}

	public SelectDeliveryView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public SelectDeliveryView(Context context, AttributeSet attrs, int defStyleAttr) {
	 ...
	 * @return
	 */
	public SelectDeliveryView fontColor(int fontColor) {
	 ...
	 * @return
	 */
	public SelectDeliveryView fontSize(int fontSize) {
	 ...
	 * @return
	 */
	public SelectDeliveryView padding(int padding) {
	 ...
	 * @return
	 */
	public SelectDeliveryView selectTip(String selectTip) {
	 ...
	 * @return
	 */
	public SelectDeliveryView lists(List<String> lists) {
	 ...
	 * @return
	 */
	public SelectDeliveryView showCount(int showCount) {
		if (showCount % 2 == 0) {
	 ...
	 * @return
	 */
	public SelectDeliveryView select(int select) {
		this.select = select;
		return this;
	}
	 ...
	 * @return
	 */
	public SelectDeliveryView build() {
		if (lists == null) {
			throw new IllegalStateException(
					"this method must invoke after the method [lists]");
		}
		return this;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// 初始化WheelSelect
		wheelSelect = new WheelSelect(showCount / 2 * itemHeight, width,
	...
	private void initWheelItems(int width, int itemHeight) {
		...
		for (int i = 0; i < showCount + 2; i++) {
			...
			wheelItems.add(new WheelItem(startY, width, itemHeight, fontColor,
					fontSize, lists.get(stringIndex)));
		}
	}
	...
	private void adjust() {
		...
		if (wheelItems.get(0).getStartY() <= (-itemHeight / 2 - itemHeight)) {
			...
			// 设置起点Y坐标
			item.setStartY(wheelItems.get(wheelItems.size() - 1).getStartY()
					+ itemHeight);
			// 得到文本在容器中的索引
			int index = lists.indexOf(wheelItems.get(wheelItems.size() - 1)
					.getText());
			if (index == -1) {
				return;
			}
	 ...
	 * @return
	 */
	public SelectDeliveryView listener(OnSelectTimeViewItemSelectListener listener) {
		this.listener = listener;
		return this;
	}

	public interface OnSelectTimeViewItemSelectListener {
