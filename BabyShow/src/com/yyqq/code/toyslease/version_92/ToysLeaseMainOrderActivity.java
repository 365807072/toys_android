...
import com.yyqq.code.toyslease.ToysLeaseMyCardActivity;
import com.yyqq.code.toyslease.ToysLeaseOrderDetailActivity;
import com.yyqq.code.toyslease.ToysLeasePayConfirmActivity;
import com.yyqq.code.toyslease.h5.ToysLeaseHtmlCollectionActivity;
...
public class ToysLeaseMainOrderActivity extends BaseActivity implements OnPullDownListener{
	...
	private LinearLayout toys_order_myprice;
	...
	private ImageView order_bespeak;
	private ImageView order_collection;
	private ImageView order_deposit;
	private ImageView order_mycard;
	...
	protected void initView() {
		...
		order_bespeak = (ImageView) findViewById(R.id.order_bespeak);
		order_collection = (ImageView) findViewById(R.id.order_collection);
		order_deposit = (ImageView) findViewById(R.id.order_deposit);
		order_mycard = (ImageView) findViewById(R.id.order_mycard);
		...
		toys_order_myprice = (LinearLayout) findViewById(R.id.toys_order_myprice);
	...
	protected void setListener() {
		...
		order_bespeak.setOnClickListener(new OnClickListener() {
		...
		order_mycard.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				if(!card_num.equals("1")){
					Intent in = new Intent(ToysLeaseMainOrderActivity.this, ToysLeaseMyCardActivity.class);
					startActivity(in);
				}else{
					Intent intent = new Intent(ToysLeaseMainOrderActivity.this, ToysLeaseOrderDetailActivity.class);
					intent.putExtra(ToysLeaseOrderDetailActivity.ORDER_ID_KEY, card_order);
					startActivity(intent);
				}
			}
		});
		
		order_deposit.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				Intent in = new Intent(ToysLeaseMainOrderActivity.this,ToysLeaseDepositActivity.class);
				startActivity(in);
			}
		});
		
		order_collection.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				Intent in = new Intent(v.getContext(), ToysLeaseHtmlCollectionActivity.class);
				startActivity(in);
			}
		});
	...
	private void initListData(String content, boolean isMore){
		try {
			if(new JSONObject(content).getBoolean("success")){
				...
				// 单卡跳详情
				card_num =  new JSONObject(content).getJSONArray("data2").getJSONObject(0).getString("num");
				if(card_num.equals("1")){
					card_order =  new JSONObject(content).getJSONArray("data2").getJSONObject(0).getString("order_id");
				}
					...
					for(int i = 0 ; i < data.length() ; i++ ){
						...
						if(!data.getJSONObject(i).getString("order_status").equals("1")){
							...
							for(int j = 0 ; j < data.getJSONObject(i).getJSONArray("toys_info").length() ; j++ ){
								...
								itemBean.setIs_prize_status(data.getJSONObject(i).getJSONArray("toys_info").getJSONObject(j).getString("is_prize_status"));
						...
						lease_main_right.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								...
								if(!MyApplication.getVisitor().equals("0")  || Config.getUser(ToysLeaseMainOrderActivity.this).uid.equals("")){
									...
									startActivity(new Intent(ToysLeaseMainOrderActivity.this, InviteFriendActivity.class));
								}
							}
						});
					}
