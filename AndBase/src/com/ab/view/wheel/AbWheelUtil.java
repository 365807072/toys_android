...
//                    textDMDateList.add(defaultYear + "-" + i + "-" + j);
                    textDMDateList.add( AbStrUtil.strFormat2(i+"") + "月" + AbStrUtil.strFormat2(j+"")+"日");
                }
            }
            else
            {
...
//                String val = AbStrUtil.dateTimeFormat(dmStr + " " + index2 + ":" + index3 + ":" + second);
                ...
                mText.setText(dmStr + " " + AbStrUtil.strFormat2(index2+"") + ":" + AbStrUtil.strFormat2(index3+""));
            }
        });
        cancelBtn.setOnClickListener(new OnClickListener()
        {
     ...
     * @param initStart the init start
     */
    public static void initWheelTimePicker2(final Activity activity,final TextView mText,final AbWheelView mWheelViewHH,final AbWheelView mWheelViewMM,Button okBtn,Button cancelBtn,int defaultHour,int defaultMinute,boolean initStart)
    {
        Calendar calendar = Calendar.getInstance();
        int hour = calendar.get(Calendar.HOUR_OF_DAY);
        int minute = calendar.get(Calendar.MINUTE);
