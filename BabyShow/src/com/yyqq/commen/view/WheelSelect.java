...
public class WheelSelect {
	...
	public WheelSelect(int startY, int width, int height, String selectText,
			int fontColor, int fontSize, int padding) {
		this.startY = startY;
		this.width = width;
		this.height = height;
	...
	public void onDraw(Canvas mCanvas) {
		...
		if (selectText != null) {
			...
			int baseLine = (int) (rect.centerY()
					+ (metrics.bottom - metrics.top) / 2 - metrics.bottom);
			// 在靠右边绘制文本
			mCanvas.drawText(selectText, rect.right - padding - textWidth,
					baseLine, mPaint);
		}
	}
}