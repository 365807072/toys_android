...
public class PhotoActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < BimpUtil.bmp.size(); i++) {
			bmp.add(BimpUtil.bmp.get(i));
		}
		for (int i = 0; i < BimpUtil.drr.size(); i++) {
			drr.add(BimpUtil.drr.get(i));
			// time.add(Bimp.time.get(i));
		}
	...
	public void startRotate(float d) {
		...
		Matrix m = new Matrix();
		m.setRotate(degrees);
		Bitmap tb = Bitmap.createBitmap(bmp.get(count), 0, 0, bmp.get(count)
				.getWidth(), bmp.get(count).getHeight(), m, false);
		bmp.set(count, tb);
		pager.removeAllViews();
		listViews.clear();
		for (int i = 0; i < bmp.size(); i++) {
		...
		try {
			FileOutputStream fos = new FileOutputStream(drr.get(count));
			bmp.get(count).compress(CompressFormat.JPEG, 100, fos);
			fos.flush();
			fos.close();
