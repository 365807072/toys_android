...
public class ToysLeaseMyCardActivity extends BaseActivity {
	...
	protected void initView() {
		...
		webView.loadUrl("https://api.meimei.yihaoss.top/H5/toy_count/cardList.html?login_user_id="
				+ Config.getUser(ToysLeaseMyCardActivity.this).uid + "&category_id="
				+ getIntent().getStringExtra(CATEGORY_ID_KEY));
		urlList.add("https://api.meimei.yihaoss.top/H5/toy_count/cardList.html?login_user_id="
				+ Config.getUser(ToysLeaseMyCardActivity.this).uid + "&category_id="
				+ getIntent().getStringExtra(CATEGORY_ID_KEY));
	}
		...
		if ((keyCode == KeyEvent.KEYCODE_BACK) && webView.canGoBack()) {
			...
			if (urlList.size() > 1) {
				if (urlList.get(urlList.size() - 1)
						.contains("https://api.meimei.yihaoss.top/H5/toy_count/cardList.html")) {
					finishThisPage();
