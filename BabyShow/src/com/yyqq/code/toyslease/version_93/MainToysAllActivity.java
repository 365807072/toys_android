...
public class MainToysAllActivity extends BaseActivity implements OnPullDownListener {
	...
	protected void initData() {
		...
		waitAdapter = new ToysLeaseMainAllAdapter(MainToysAllActivity.this, inflater, MyApplication.getInstance(), data, 0, ab, (int) ((int) am.widthPixels * 0.4));
	...
	protected void setListener() {
		...
		lease_select_list04_ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!toys_type.equals("")){
					...
					toys_top_type04_text.setText("筛选");
				}
				initWaitData(false);
		...
		search_to_cart.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(MainToysAllActivity.this, ToysLeaseMainShoppingCartActivity.class);
...
//		}
		...
		// URL 中有空格的话需要转义： URL中的空格 = %20
		String url = ServerMutualConfig.GET_LEASE_MAIN_LIST + "&" + params.toString();
		
		if(url.contains(" ")){
			url = url.replaceAll(" ", "%20");
		}
		
		ab.get(url, new AbStringHttpResponseListener() {
			...
			if (!content.equals("") && new JSONObject(content).getBoolean("success")) {
				...
				if(json.has("data2")){
					if(!json.getString("data2").equals("")){
						...
						shopping_car_hint_text.setText(json.getString("data2"));//如有疑问 需要更改的话
					}else{
				...
				if (!isMore) {
					...
					if (json.getJSONArray("data").length() != 20) {
						...
						if(json.getJSONArray("data").length() == 0){
							waitAdapter.notifyDataSetChanged();
							Toast.makeText(MainToysAllActivity.this, "未找到符合您要求的玩具", 2).show();
							return;
						}else{
							Toast.makeText(MainToysAllActivity.this, "已显示所有符合条件的玩具", 2).show();
						}
					}else{
						mPullDownView.setShowFooter();
					}
								...
								lease_main_right.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
										...
										if(!MyApplication.getVisitor().equals("0")  || Config.getUser(MainToysAllActivity.this).uid.equals("")){
											...
											startActivity(new Intent(MainToysAllActivity.this, InviteFriendActivity.class));
										}
									}
								});
							}
