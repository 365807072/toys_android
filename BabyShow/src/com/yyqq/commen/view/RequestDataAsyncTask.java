...
public class RequestDataAsyncTask {
	...
	private int notDataImgResId = R.drawable.def_head;
	 ...
	 * 
	 * @param isShowNoDataLayout
	 *            true展示，false不展示 默认false
	 */
	...
	public RequestDataAsyncTask(Context context,
			PullToRefreshView pullToRefreshView, AbsListView listView) {
	...
	public void executeLoadData() {
		if (NetWorkUtil.netWorkConnection(mContext)) {
			...
			Toast.makeText(mContext, "网络连接异常", 3).show();
	 ...
	 * 
	 */
	public class RequestWebDataHelpr extends
			AsyncTask<Void, Void, List<Object>> {
		// 是否继续执行标志
		private boolean executedata = true;
		/**
		 * 判断异步类是否在执行 当返回为false的时候则表示正在执行 反之没执行
		 */
	...
	public void initDataView(int size) {
		if (isShowNoDataLayout) {
			if (null == notDataView) {
				// 初始化没有数据时候显示的UI
				notDataView = LayoutInflater.from(mContext).inflate(
						R.layout.comment_no_data_layout, null);
				TextView tvNotDataTxt = (TextView) notDataView
						.findViewById(R.id.tvNotDataTxt);
				tvNotDataTxt.setText(notDataMsg);
				ImageView ivNotDataImg = (ImageView) notDataView
						.findViewById(R.id.ivNotDataDefaultImg);
				ivNotDataImg.setImageResource(notDataImgResId);
				pullToRefreshView.addView(notDataView, new LayoutParams(
