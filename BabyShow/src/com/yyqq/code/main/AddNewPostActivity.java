...
public class AddNewPostActivity extends Activity {
	...
	private boolean isFromGroup = false; // 是否从群内点击发帖过来
	private String get_group_id = "";
	...
	private LinearLayout add_to_group;
	private Button add_to_group_btn;
	private TextView add_to_group_text;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		context = this;
		// 有group_id表示从群内跳转过来的，无需获取
		if(getIntent().hasExtra("group_id")){
			isFromGroup = true;
			group_id = getIntent().getStringExtra("group_id");
			add_to_group = (LinearLayout) findViewById(R.id.add_to_group);
			add_to_group.setVisibility(View.GONE);
		}else{ // 从首页或其他页面跳转过来的，需要群信息
			isFromGroup = false;
			getGroupInfo();
		}
		// 消息通知
					...
					new View.OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							if(video_path.isEmpty()){
								...
								notification.setLatestEventInfo(context, "自由环球租赁", "正在发布", null);
				...
				fh.post(url, params, new AjaxCallBack<String>() {
							...
							public void onSuccess(String t) {
								...
								if(!tag_id.isEmpty()){
									...
									notification.setLatestEventInfo(context, "自由环球租赁", "发布成功！", null);
...
//			isSelectSina = false;
	...
	private void shareSinaWeibo(String shearUrlImg, String msg, String path) {
		...
		String url = "";
		if(video_path.isEmpty()){
			url = "http://www.meimei.yihaoss.top/fenxiang/postbardetial.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id;
		}else{
			url = "http://www.meimei.yihaoss.top/fenxiang/video.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id;
		}
		...
		if (TextUtils.isEmpty(path) || path.startsWith("http")) {
			...
			if(video_path.isEmpty()){
				sp.setImageUrl("http://www.meimei.yihaoss.top/fenxiang/postbardetial.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id);
			}else{
				sp.setImageUrl("http://www.meimei.yihaoss.top/fenxiang/video.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id);
			}
		}
	...
	private void shareWechat(String shearUrlImg, String msg, String path) {
		...
		if (TextUtils.isEmpty(path) || path.startsWith("http")) {
			...
			if(video_path.isEmpty()){
				...
				sp.setImageUrl("http://www.meimei.yihaoss.top/fenxiang/index.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id);
			}
		}
		
		if(video_path.isEmpty()){
			sp.setUrl("http://www.meimei.yihaoss.top/fenxiang/postbardetial.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id);
		}else{
			sp.setUrl("http://www.meimei.yihaoss.top/fenxiang/video.html?" + "img_id=" + item.img_id + "&user_id=" + item.user_id);
		}
	...
	private void getGroupInfo(){
		add_to_group = (LinearLayout) findViewById(R.id.add_to_group);
		add_to_group_btn = (Button) findViewById(R.id.add_to_group_btn);
		add_to_group_text = (TextView) findViewById(R.id.add_to_group_text);
		final AbRequestParams params = new AbRequestParams();
		params.put("login_user_id", Config.getUser(context).uid);
		AbHttpUtil.getInstance(context).get(ServerMutualConfig.GET_GROUP_INFO + "&" + params.toString(),new AbStringHttpResponseListener() {
			@Override
			public void onSuccess(int statusCode, String content) {
				super.onSuccess(statusCode, content);
				try {
					if(!content.isEmpty() && new JSONObject(content).getBoolean("success")){
						get_group_id = new JSONObject(content).getJSONObject("data").getString("group_id");
						group_name = new JSONObject(content).getJSONObject("data").getString("group_name");
						group_id = get_group_id;
						if(!group_name.isEmpty() && !group_id.isEmpty()){
							add_to_group.setVisibility(View.VISIBLE);
							add_to_group_text.setText(group_name);
							add_to_group_btn.setOnClickListener(new OnClickListener() {
								
								@Override
								public void onClick(View v) {
									if(group_id.isEmpty()){
										group_id = get_group_id;
										add_to_group_btn.setBackgroundResource(R.drawable.sel_bt);
									}else{
										group_id = "";
										add_to_group_btn.setBackgroundResource(R.drawable.sel_bt_no);
									}
								}
							});
						}else{
