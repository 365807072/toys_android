package com.yyqq.code.toyslease.version_94;
...
import com.yyqq.code.toyslease.version_93.SearchMainActivity;
 ...
 * 玩具世界首页
 * */
public class NewMainToysHomeActivity extends BaseActivity  implements OnPullDownListener{
		...
		toys_head_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				if(heanList.get(position).getIs_jump().equals("0")){
					Intent intent = new Intent(NewMainToysHomeActivity.this, ToysLeaseDetailActivity.class);
					intent.putExtra(ToysLeaseDetailActivity.TOYS_DETAIL_KEY, heanList.get(position).getId());
					NewMainToysHomeActivity.this.startActivity(intent);
				}else{
					Intent intent = new Intent(NewMainToysHomeActivity.this, MainItemDetialWebActivity.class);
...
//		});
		...
		search_root.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				startActivity(new Intent(NewMainToysHomeActivity.this, SearchMainActivity.class));
			}
		});
...
//					TO_CHANGE_TITLE = true;
		...
		main_head_search.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(NewMainToysHomeActivity.this, SearchMainActivity.class);
				NewMainToysHomeActivity.this.startActivity(intent);
			}
		});
		...
		findViewById(R.id.find).setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				NewMainToysHomeActivity.this.finish();
			}
		});
...
//		getMainData(true);
	...
	protected void onResume() {
		...
		MainTab.iconText.get(2).setTextColor(Color.parseColor("#000000"));
	...
	private void addPoint() {
		...
		for (int i = 0; i < BannerList.size(); i++) {
			mPointImg = new ImageView(NewMainToysHomeActivity.this);
	...
	public void getHeadList() {
		...
		ab.get(ServerMutualConfig.GET_TOYS_BANNER_list + "&" + params.toString(), new AbStringHttpResponseListener() {
					...
					public void onFinish() {
						...
						if(null !=  NewMainToysHomeActivity.search_root){
							NewMainToysHomeActivity.search_root.setVisibility(View.VISIBLE);
						}
					}
				...
				try {
					if(new JSONObject(content).getBoolean("success")){
						...
						lease_main_right_02.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								 // 检查登录状态
								if(!MyApplication.getVisitor().equals("0")  || Config.getUser(NewMainToysHomeActivity.this).uid.equals("")){
									Intent in = new Intent(NewMainToysHomeActivity.this, WebLoginActivity.class);
									startActivity(in);
								}else{
									startActivity(new Intent(NewMainToysHomeActivity.this, InviteFriendActivity.class));
								}
							}
						});
					}
